...
let isInTarget = currentHR > targetLowerBound
let isAboveTarget = currentHR < targetUpperBound
if isInTarget == true && isAboveTarget == true {
    print( "Вы на верном пути!")
}else if isInTarget == false {
    print("У вас все отлично, но попробуйте немного подтолкнуть!")
}else if isAboveTarget == false {
    print("Вы в огне! Немного замедлите его")
}
